...
public static class ModrinthApi
{
	...
	{
		using HttpClient client = CreateClient();
		string url = $"https://api.modrinth.com/v2/project/{Uri.EscapeDataString(projectIdOrSlug)}";
		try
		{
			JsonDocument doc = JsonDocument.Parse(await client.GetStringAsync(url));
			...
			ModrinthProject proj = new ModrinthProject
			{
				ProjectId = root.TryGetProperty("id", out var idEl) ? idEl.GetString() ?? "" : "",
				Slug = root.TryGetProperty("slug", out var slugEl) ? slugEl.GetString() ?? "" : "",
				...
				IconUrl = root.TryGetProperty("icon_url", out var iconEl) ? iconEl.GetString() ?? "" : "",
				Downloads = root.TryGetProperty("downloads", out var dlEl) ? dlEl.GetInt64() : 0
			};
			if (root.TryGetProperty("versions", out var vers))
			{
				proj.GameVersions = (from v in vers.EnumerateArray() select v.GetString() ?? "" into v where !string.IsNullOrEmpty(v) select v).ToList();
			}
			if (root.TryGetProperty("categories", out var cats))
			{
				proj.Loaders = (from v in cats.EnumerateArray() select v.GetString() ?? "" into v where !string.IsNullOrEmpty(v) select v).ToList();
			}
			return proj;
		}
		catch
	...
	public static async Task<List<ModrinthVersion>> GetProjectVersions(string projectId)
	{
		...
		foreach (JsonElement versionElem in doc.RootElement.EnumerateArray())
		{
			...
			if (versionElem.TryGetProperty("dependencies", out var depsEl))
			{
				foreach (var depEl in depsEl.EnumerateArray())
				{
					var dep = new ModrinthDependency
					{
						ProjectId = depEl.TryGetProperty("project_id", out var pidEl) ? pidEl.GetString() ?? "" : "",
						VersionId = depEl.TryGetProperty("version_id", out var vidEl) ? vidEl.GetString() ?? "" : "",
						DependencyType = depEl.TryGetProperty("dependency_type", out var dtEl) ? dtEl.GetString() ?? "required" : "required"
					};
	...
	public static async Task<Dictionary<string, string>> GetProjectNamesAsync(IEnumerable<string> projectIds)
	{
		var result = new Dictionary<string, string>();
		var idList = projectIds.Where(id => !string.IsNullOrEmpty(id)).Distinct().ToList();
		if (idList.Count == 0)
			return result;
		using HttpClient client = CreateClient();
		string idsJson = JsonSerializer.Serialize(idList);
		string url = $"https://api.modrinth.com/v2/projects?ids={Uri.EscapeDataString(idsJson)}";
		try
		{
			JsonDocument doc = JsonDocument.Parse(await client.GetStringAsync(url));
			foreach (JsonElement proj in doc.RootElement.EnumerateArray())
			{
				string id = proj.TryGetProperty("id", out var idEl) ? idEl.GetString() ?? "" : "";
				string title = proj.TryGetProperty("title", out var titleEl) ? titleEl.GetString() ?? "" : "";
