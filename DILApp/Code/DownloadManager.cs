...
{
	private static readonly List<DownloadTask> _tasks = new List<DownloadTask>();
	...
	public static DownloadTask? CurrentTask
	{
		get
		{
			lock (_tasksLock)
			{
				return _tasks.FirstOrDefault(t => t.IsActive);
			}
		}
	}

	public static IReadOnlyList<DownloadTask> AllTasks
	{
		get
		{
			lock (_tasksLock)
	...
	public static bool IsDownloading
	{
		get
		{
			lock (_tasksLock)
			{
				return _tasks.Any(t => t.IsActive);
			}
		}
	}

	public static string MinecraftPath { get; set; } = Path.Combine(GetExeDirectory(), "minecraft");
	...
	private static async Task DownloadFileWithProgressAsync(DownloadTask task, HttpClient client, string url, string filePath, int timeoutSec = 120, string? label = null)
	{
		Exception lastEx = null;
		for (int attempt = 1; attempt <= 3; attempt++)
		{
				...
				try
				{
					...
					try
					{
						...
						while (true)
						{
							...
							totalRead += read;
							Interlocked.Exchange(ref task.CurrentBytes, totalRead);
							if (contentLength <= 0 || task == null)
							{
								continue;
							}
							...
							lock (task)
							{
								...
								{
									task.StepText = $"{label} ({FormatBytes(totalRead)} / {FormatBytes(contentLength)})";
								}
								DownloadManager.ProgressChanged?.Invoke(task);
							}
						}
					}
					finally
	...
	private static bool ValidateFileSize(string filePath, long expectedSize)
	{
		if (expectedSize <= 0 || !File.Exists(filePath))
		{
			return false;
		}
		try
	...
	public static void StartDownload(string versionId, string loaderName = "", string loaderVersion = "", string optifineVersion = "")
	{
		...
		DownloadManager.ProgressChanged?.Invoke(task);
		EnsureSpeedMonitor();
		DownloadAsync(task, versionId, loaderName, loaderVersion, optifineVersion);
	}

	public static void CancelDownload(int? taskId = null)
	{
		lock (_tasksLock)
		{
			var targets = taskId.HasValue
				? _tasks.Where(t => t.Id == taskId.Value && t.IsActive).ToList()
				: _tasks.Where(t => t.IsActive).ToList();
			foreach (var t in targets)
			{
				t.CancellationTokenSource.Cancel();
				t.Step = DownloadStep.Cancelled;
				t.StepText = LanguageManager.Get("DlCancelled");
				t.Speed = 0.0;
			}
		}
		if (!IsDownloading)
		...
		if (current != null)
		{
			DownloadManager.ProgressChanged?.Invoke(current);
		}
	}
	...
	public static void StartResourceDownload(string name, string type, Func<Task> downloadAction)
	{
		...
		DownloadManager.ProgressChanged?.Invoke(task);
		EnsureSpeedMonitor();
		DownloadResourceAsync(task, downloadAction);
	}

	private static async Task DownloadResourceAsync(DownloadTask task, Func<Task> downloadAction)
	{
		try
		{
			UpdateStep(task, DownloadStep.DownloadingClient, LanguageManager.Get("DlDownloadingFile"), 10.0, 0, 1);
			await downloadAction();
			UpdateStep(task, DownloadStep.Completed, LanguageManager.Get("DlComplete"), 100.0, 1, 1);
			if (!IsDownloading)
			{
				_speedTimer?.Stop();
			}
			task.Speed = 0.0;
			DownloadManager.DownloadCompleted?.Invoke(task);
		}
		catch (Exception ex2)
		{
			Exception ex = ex2;
			...
			task.Step = DownloadStep.Failed;
			task.StepText = string.Format(LanguageManager.Get("DlFailed"), ex.Message);
			DownloadManager.DownloadFailed?.Invoke(task, ex);
		}
	}

	private static void EnsureSpeedMonitor()
	{
		...
		_speedTimer.Tick += delegate
		{
			...
			lock (_tasksLock)
			{
				activeTasks = _tasks.Where(t => t.IsActive).ToList();
			}
			...
			{
				double totalSeconds = (now - t.LastSpeedUpdate).TotalSeconds;
				if (totalSeconds > 0.0)
				{
					long num = t.CurrentBytes - t.LastBytes;
					if (num >= 0)
					{
				...
				DownloadManager.ProgressChanged?.Invoke(t);
			}
		};
		_speedTimer.Start();
	}

	private static async Task DownloadAsync(DownloadTask task, string versionId, string loaderName, string loaderVersion, string optifineVersion)
	{
		try
		{
			using HttpClient client = CreateClient();
			UpdateStep(task, DownloadStep.DownloadingJson, LanguageManager.Get("DlVersionManifest"), 0.0, 0, 1);
			string versionJson = await DownloadVersionJson(client, versionId);
			task.CurrentBytes += versionJson.Length;
			...
			if (isModdedVersion)
			{
				...
				if (shouldSkipBaseDownload)
				{
					if (task != null)
					{
						task.StepText = string.Format(LanguageManager.Get("DlBaseVersionExists"), versionId);
					}
					DownloadManager.ProgressChanged?.Invoke(task);
			...
			if (!shouldSkipBaseDownload)
			{
				UpdateStep(task, DownloadStep.DownloadingClient, LanguageManager.Get("DlClient"), 5.0, 0, 1);
				await DownloadClientJar(task, client, versionId, versionJson, versionDir);
				UpdateStep(task, DownloadStep.DownloadingLibraries, LanguageManager.Get("DlLibraries"), 20.0, 0, 0);
				await DownloadLibraries(task, client, versionJson);
				UpdateStep(task, DownloadStep.DownloadingAssets, LanguageManager.Get("DlAssets"), 50.0, 0, 0);
				await DownloadAssets(task, client, versionJson);
			}
			else
			{
			...
			{
				UpdateStep(task, DownloadStep.DownloadingModLoader, string.Format(arg0: string.IsNullOrEmpty(loaderVersion) ? loaderName : (loaderName + " " + loaderVersion), format: LanguageManager.Get("DlModLoader")), 85.0, 0, 0);
				await DownloadModLoader(task, client, versionId, loaderName, loaderVersion);
			}
			if (!string.IsNullOrEmpty(optifineVersion))
		{
			UpdateStep(task, DownloadStep.DownloadingModLoader, string.Format("Optifine {0}", optifineVersion), 92.0, 0, 0);
			bool hasMainLoader = !string.IsNullOrEmpty(loaderName) && loaderName != "Optifine";
			string inheritsFrom = hasMainLoader
				? $"{versionId}-{loaderName.ToLower()}-{loaderVersion}"
				: versionId;
			await InstallOptifineLoader(task, client, versionId, optifineVersion, inheritsFrom, hasMainLoader);
		}
			UpdateStep(task, DownloadStep.Completed, LanguageManager.Get("DlComplete"), 100.0, 0, 0);
			_speedTimer?.Stop();
			if (task != null)
			{
				task.Speed = 0.0;
	...
	private static void UpdateStep(DownloadTask task, DownloadStep step, string text, double progress, int currentFile, int totalFiles)
	{
		task.Step = step;
		task.StepText = text;
		task.Progress = progress;
		task.CurrentFileIndex = currentFile;
	...
	private static async Task DownloadClientJar(DownloadTask task, HttpClient client, string versionId, string versionJson, string versionDir)
	{
		...
		while (true)
		{
			...
			if (task != null)
			{
				if (totalBytes > 0)
				{
					task.Progress = 5.0 + (double)totalRead * 15.0 / (double)totalBytes;
				}
				task.StepText = string.Format(LanguageManager.Get("DlClientProgress"), (double)totalRead / 1024.0 / 1024.0, (double)totalBytes / 1024.0 / 1024.0);
				DownloadManager.ProgressChanged?.Invoke(task);
			}
		}
	}

	private static async Task DownloadLibrariesWithRetry(DownloadTask task, HttpClient client, string versionJson)
	{
		...
		try
		{
			...
			foreach (var item in libList)
			{
				...
				tasks.Add(Task.Run(async delegate
				{
					try
					{
						...
						{
							DateTime now = DateTime.Now;
							if ((now - task.LastSpeedUpdate).TotalMilliseconds >= 200.0)
							{
								task.LastSpeedUpdate = now;
								lock (task)
								{
									task.Progress = 20.0 + (double)done * 30.0 / (double)total;
									task.CurrentFileIndex = done;
									task.TotalFiles = total;
									if (failedCount > 0 || done == total)
									{
										task.StepText = string.Format(LanguageManager.Get("DlLibrariesProgressFailed"), done, total, failedCount);
									}
									else
									{
										task.StepText = string.Format(LanguageManager.Get("DlLibrariesProgress"), done, total);
									}
									DownloadManager.ProgressChanged?.Invoke(task);
								}
							}
			...
			if (task != null)
			{
				task.StepText = string.Format(LanguageManager.Get("DlRetrying"), failedFiles.Count);
				DownloadManager.ProgressChanged?.Invoke(task);
			}
			int retryFailed = 0;
	...
	private static async Task DownloadLibraries(DownloadTask task, HttpClient client, string versionJson)
	{
		HttpClient client2 = client;
		JsonDocument doc = JsonDocument.Parse(versionJson);
		string libDir = Path.Combine(MinecraftPath, "libraries");
		...
		try
		{
					...
					try
					{
						...
						if (!File.Exists(localPath2))
						{
							try
							{
								...
								Interlocked.Add(ref task.CurrentBytes, data2.Length);
							}
							catch
							{
								Interlocked.Increment(ref failedCount);
						...
						if (task != null && total > 0)
						{
							lock (task)
							{
								...
								task.CurrentFileIndex = done;
								task.TotalFiles = total;
								task.StepText = ((failedCount > 0) ? string.Format(LanguageManager.Get("DlLibrariesProgressFailed"), done, total, failedCount) : string.Format(LanguageManager.Get("DlLibrariesProgress"), done, total));
								DownloadManager.ProgressChanged?.Invoke(task);
							}
						}
					}
			...
			foreach (var item2 in failedFiles)
			{
				...
				try
				{
					...
					Interlocked.Add(ref task.CurrentBytes, data.Length);
					retrySuccess++;
				}
				catch
				{
	...
	private static async Task DownloadAssets(DownloadTask task, HttpClient client, string versionJson)
	{
		HttpClient client2 = client;
		JsonDocument doc = JsonDocument.Parse(versionJson);
		string assetIndexUrl = null;
		string assetIndexId = "1.20";
		long assetIndexSize = 0L;
		if (doc.RootElement.TryGetProperty("assetIndex", out var ai))
		{
			...
			if (ai.TryGetProperty("size", out var sizeProp2) && sizeProp2.TryGetInt64(out var sz2))
			{
				assetIndexSize = sz2;
			}
		}
		...
		if (!File.Exists(indexPath) || (assetIndexSize > 0 && !ValidateFileSize(indexPath, assetIndexSize)))
		{
			string indexJson = await TryGetString(client2, assetIndexUrl);
			await File.WriteAllTextAsync(indexPath, indexJson);
			Interlocked.Add(ref task.CurrentBytes, indexJson.Length);
		}
		...
		try
		{
			...
			if (task != null)
			{
				task.TotalFiles = total;
				task.CurrentFileIndex = 0;
				task.StepText = string.Format(LanguageManager.Get("DlAssetsProgress"), 0, total);
				DownloadManager.ProgressChanged?.Invoke(task);
			}
			foreach (var item in assetList)
			{
				...
				tasks.Add(Task.Run(async delegate
				{
					try
					{
						...
						{
							DateTime now = DateTime.Now;
							if ((now - task.LastSpeedUpdate).TotalMilliseconds >= 150.0)
							{
								task.LastSpeedUpdate = now;
								lock (task)
								{
									task.Progress = 50.0 + (double)done * 35.0 / (double)total;
									task.CurrentFileIndex = done;
									task.TotalFiles = total;
									if (failedCount > 0 || done == total)
									{
										task.StepText = string.Format(LanguageManager.Get("DlAssetsProgressFailed"), done, total, failedCount);
									}
									else
									{
										task.StepText = string.Format(LanguageManager.Get("DlAssetsProgress"), done, total);
									}
									DownloadManager.ProgressChanged?.Invoke(task);
								}
							}
			...
			if (task != null)
			{
				task.StepText = string.Format(LanguageManager.Get("DlRetrying"), failedAssets.Count);
				DownloadManager.ProgressChanged?.Invoke(task);
			}
			int retryFailed = 0;
	...
	private static async Task DownloadModLoader(DownloadTask task, HttpClient client, string versionId, string loaderName, string loaderVersion)
	{
		...
		{
		case "Fabric":
			await InstallFabricLoader(task, client, versionId, loaderVersion);
			break;
		case "Quilt":
			await InstallQuiltLoader(task, client, versionId, loaderVersion);
			break;
		case "Optifine":
			await InstallOptifineLoader(task, client, versionId, loaderVersion, versionId);
			break;
		default:
		...
		case "Forge":
			...
			if (num != 0)
			{
				await InstallForgeLoader(task, client, versionId, loaderVersion, loaderName);
			}
			break;
		}
		if (task != null)
		{
			task.StepText = string.Format(LanguageManager.Get("DlLoaderInstalled"), loaderName);
			DownloadManager.ProgressChanged?.Invoke(task);
		}
	}

	private static async Task InstallFabricLoader(DownloadTask task, HttpClient client, string gameVersion, string loaderVersion)
	{
		...
		if (task != null)
		{
			task.StepText = string.Format(LanguageManager.Get("DlModLoader"), "Fabric");
			DownloadManager.ProgressChanged?.Invoke(task);
		}
		await DownloadLibrariesWithRetry(task, client, profileJson);
		await DownloadAssets(task, client, profileJson);
	}

	private static async Task InstallQuiltLoader(DownloadTask task, HttpClient client, string gameVersion, string loaderVersion)
	{
		...
		if (task != null)
		{
			task.StepText = string.Format(LanguageManager.Get("DlModLoader"), "Quilt");
			DownloadManager.ProgressChanged?.Invoke(task);
		}
		await DownloadLibrariesWithRetry(task, client, profileJson);
		await DownloadAssets(task, client, profileJson);
	...
	private static async Task InstallForgeLoader(DownloadTask task, HttpClient client, string gameVersion, string loaderVersion, string loaderName)
	{
		...
		if (!File.Exists(installerPath) || new FileInfo(installerPath).Length == 0)
		{
			if (task != null)
			{
				task.StepText = string.Format(LanguageManager.Get("DlModLoader"), loaderName);
				task.TotalBytes = 0L;
			...
			foreach (string url in candidateUrls)
			{
				try
				{
					await DownloadFileWithProgressAsync(task, client, url, installerPath, 120, dlLabel);
		...
		await PreDownloadForgeLibraries(task, client, installerPath);
		EnsureLauncherProfiles();
		if (task != null)
		{
			task.StepText = string.Format(LanguageManager.Get("DlForgeInstalling"), loaderName);
			DownloadManager.ProgressChanged?.Invoke(task);
		}
			ProcessStartInfo psi = new ProcessStartInfo
		{
			FileName = javaPath,
			Arguments = $"-Dfile.encoding=UTF-8 -Dsun.jnu.encoding=UTF-8 -Dmirror=https://bmclapi2.bangbang93.com/maven/ -Dforge.don't.prompt=true -jar \"{installerPath}\" --installClient \"{MinecraftPath}\"",
		...
		try
		{
			...
			await PostDownloadForgeRuntimeLibraries(task, client, installerPath);
			string versionDir = Path.Combine(MinecraftPath, "versions", task.VersionId ?? "");
			string versionJsonFile = Path.Combine(versionDir, task.VersionId + ".json");
			if (File.Exists(versionJsonFile))
			{
				await DownloadAssets(task, client, await File.ReadAllTextAsync(versionJsonFile));
			}
		}
		finally
		{
	...
	private static async Task PreDownloadForgeLibraries(DownloadTask task, HttpClient client, string installerPath)
	{
		HttpClient client2 = client;
		...
		if (task != null)
		{
			task.StepText = string.Format(LanguageManager.Get("DlForgePreDownload"), 0, total);
			task.TotalFiles = total;
			DownloadManager.ProgressChanged?.Invoke(task);
		}
			...
			finally
			{
				...
				if (task != null && total > 0)
				{
					lock (task)
					{
						task.CurrentFileIndex = done;
						task.TotalFiles = total;
						task.StepText = string.Format(LanguageManager.Get("DlForgePreDownload"), done, total);
						DownloadManager.ProgressChanged?.Invoke(task);
					}
				}
			}
		...
		await Task.WhenAll(tasks);
		if (failedFiles.Count <= 0 || task == null)
		{
			return;
		}
		task.StepText = string.Format(LanguageManager.Get("DlRetrying"), failedFiles.Count);
	...
	private static async Task PostDownloadForgeRuntimeLibraries(DownloadTask task, HttpClient client, string installerPath)
	{
		HttpClient client2 = client;
		string librariesDir = Path.Combine(MinecraftPath, "libraries");
		Directory.CreateDirectory(librariesDir);
		...
		try
		{
			...
			if (versionEntry != null)
			{
				...
				JsonDocument jsonDocument = JsonDocument.Parse(await streamReader.ReadToEndAsync());
				if (jsonDocument.RootElement.TryGetProperty("libraries", out var libs2) && libs2.ValueKind == JsonValueKind.Array)
				{
					runtimeLibsElement = libs2.Clone();
				}
			}
			if (!runtimeLibsElement.HasValue)
			{
				...
				if (profileEntry != null)
				{
					...
					using StreamReader r = new StreamReader(s, Encoding.UTF8);
					JsonDocument doc = JsonDocument.Parse(await r.ReadToEndAsync());
		...
		if (task != null)
		{
			task.StepText = string.Format(LanguageManager.Get("DlForgeRuntimeLibs"), 0, total);
			task.TotalFiles = total;
			DownloadManager.ProgressChanged?.Invoke(task);
		}
		IEnumerable<Task> tasks = libList.Select<(string, string), Task>(async delegate((string url, string path) item)
			...
			if (task != null && total > 0)
			{
				...
				if ((now - task.LastSpeedUpdate).TotalMilliseconds >= 200.0)
				{
					...
					lock (task)
					{
						...
						if (failedCount > 0 || done == total)
						{
							task.StepText = string.Format(LanguageManager.Get("DlForgeRuntimeLibsFailed"), done, total, failedCount);
						}
						else
						{
							task.StepText = string.Format(LanguageManager.Get("DlForgeRuntimeLibs"), done, total);
						}
						DownloadManager.ProgressChanged?.Invoke(task);
					}
				}
	...
	public static async Task EnsureBaseJarAsync(string versionId)
	{
		...
		await DownloadClientJarSimple(client, jarVersion, baseJson, jarDir);
	}

	private static async Task DownloadClientJarSimple(HttpClient client, string versionId, string versionJson, string versionDir)
	{
		JsonDocument vDoc = JsonDocument.Parse(versionJson);
		string clientUrl = null;
		if (vDoc.RootElement.TryGetProperty("downloads", out var downloads) && downloads.TryGetProperty("client", out var clientInfo) && clientInfo.TryGetProperty("url", out var urlProp))
	...
	internal static string FindJavaPath()
	{
		if (!string.IsNullOrEmpty(LauncherConfig.Current.CustomJavaPath) && File.Exists(LauncherConfig.Current.CustomJavaPath))
		{
			return LauncherConfig.Current.CustomJavaPath;
		}
		List<string> list = new List<string>();
		string environmentVariable = Environment.GetEnvironmentVariable("JAVA_HOME");
		if (!string.IsNullOrEmpty(environmentVariable))
		{
			list.Add(Path.Combine(environmentVariable, "bin", "javaw.exe"));
		...
		foreach (string text2 in array)
		{
			...
			{
				list.Add(Path.Combine(text2, "javaw.exe"));
				list.Add(Path.Combine(text2, "java.exe"));
				list.Add(Path.Combine(text2, "java"));
			}
		}
		list.Add(Path.Combine(MinecraftPath, "runtime", "jre", "bin", "javaw.exe"));
		list.Add(Path.Combine(MinecraftPath, "runtime", "jre", "bin", "java.exe"));
		string runtimeDir = Path.Combine(MinecraftPath, "runtime");
		if (Directory.Exists(runtimeDir))
		{
			try
			{
				foreach (string dir in Directory.GetDirectories(runtimeDir, "*", SearchOption.AllDirectories))
				{
	...
	internal static string FindJavaPathForVersion(string versionId)
	{
		if (!string.IsNullOrEmpty(LauncherConfig.Current.CustomJavaPath) && File.Exists(LauncherConfig.Current.CustomJavaPath))
		{
			return LauncherConfig.Current.CustomJavaPath;
		}
		int requiredMajor = GetRequiredJavaVersion(versionId);
		List<(string Path, int Version)> candidates = new List<(string, int)>();
		HashSet<string> seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
		...
		{
			searchPaths.Add(Path.Combine(envJavaHome, "bin", "javaw.exe"));
			searchPaths.Add(Path.Combine(envJavaHome, "bin", "java.exe"));
		}
		...
		foreach (string p in envPath.Split(Path.PathSeparator))
		{
			...
			{
				searchPaths.Add(Path.Combine(p, "javaw.exe"));
				searchPaths.Add(Path.Combine(p, "java.exe"));
			}
		}
		searchPaths.Add(Path.Combine(MinecraftPath, "runtime", "jre", "bin", "javaw.exe"));
		searchPaths.Add(Path.Combine(MinecraftPath, "runtime", "jre", "bin", "java.exe"));
		foreach (string path in searchPaths)
		{
			try
			{
				if (!File.Exists(path) || seen.Contains(path))
					continue;
				seen.Add(path);
				int ver = GetJavaMajorVersion(path);
				if (ver > 0)
					candidates.Add((path, ver));
			}
			catch { }
		}
		(string Path, int Version) best = ("", 0);
		foreach (var c in candidates)
		{
			if (c.Version >= requiredMajor && (best.Version == 0 || c.Version < best.Version || best.Version < requiredMajor))
				best = c;
		}
		...
		foreach (var c in candidates)
		{
			if (c.Version >= requiredMajor)
				return c.Path;
		}
		return candidates.Count > 0 ? candidates[0].Path : FindJavaPath();
	}

	private static int GetRequiredJavaVersion(string versionId)
	{
		string jsonPath = Path.Combine(MinecraftPath, "versions", versionId, $"{versionId}.json");
		if (File.Exists(jsonPath))
		{
			try
			{
				...
				if (doc.RootElement.TryGetProperty("javaVersion", out var jv) && jv.TryGetProperty("majorVersion", out var mv))
					return mv.GetInt32();
			}
			catch { }
		}
		if (versionId.Contains("1.20.5") || versionId.Contains("1.21"))
			return 21;
		if (versionId.Contains("1.17") || versionId.Contains("1.18") || versionId.Contains("1.19") || versionId.Contains("1.20"))
			return 17;
		if (versionId.Contains("1.16"))
			return 16;
		return 8;
	}
	...
	private static int GetJavaMajorVersion(string javaPath)
	{
		try
		{
			string exe = javaPath.EndsWith("javaw.exe", StringComparison.OrdinalIgnoreCase) ? javaPath : javaPath.Replace("java.exe", "javaw.exe");
			if (!File.Exists(exe))
				exe = javaPath;
			ProcessStartInfo psi = new ProcessStartInfo
			{
			...
			m = Regex.Match(output, @"version ""1\.(\d+)");
	...
	public static async Task<List<string>> GetLoaderVersionsAsync(string loaderName, string gameVersion)
	{
		...
		try
		{
			...
			List<string> result = loaderName switch
			{
				...
				"Optifine" => await GetOptifineVersionsAsync(client, gameVersion), 
				_ => new List<string>(), 
			};
			if (1 == 0)
			{
	...
	private static async Task<List<string>> GetOptifineVersionsAsync(HttpClient client, string gameVersion)
	{
		string url = $"https://bmclapi2.bangbang93.com/optifine/{gameVersion}";
		try
		{
			string json = await TryGetString(client, url);
			JsonDocument doc = JsonDocument.Parse(json);
			List<string> result = new List<string>();
			foreach (JsonElement item in doc.RootElement.EnumerateArray())
			{
				string type = item.TryGetProperty("type", out var t) ? t.GetString() ?? "" : "";
				string patch = item.TryGetProperty("patch", out var p) ? p.GetString() ?? "" : "";
				string filename = item.TryGetProperty("filename", out var f) ? f.GetString() ?? "" : "";
				string label = $"{type}_{patch}|{filename}";
				if (!string.IsNullOrEmpty(type) && !result.Any(r => r.StartsWith($"{type}_{patch}|")))
				{
					result.Add(label);
				}
			}
	...
	private static string ResolveGameJarPath(string versionId)
	{
		...
		{
			visited.Add(current);
			string jarPath = Path.Combine(MinecraftPath, "versions", current, $"{current}.jar");
			if (File.Exists(jarPath))
				return jarPath;
			string jsonPath = Path.Combine(MinecraftPath, "versions", current, $"{current}.json");
			if (!File.Exists(jsonPath))
				break;
			try
			{
		...
		return Path.Combine(MinecraftPath, "versions", versionId, $"{versionId}.jar");
	}

	private static async Task InstallOptifineLoader(DownloadTask task, HttpClient client, string gameVersion, string loaderVersion, string inheritsFrom, bool mergeIntoParent = false)
	{
		string filename = "";
		string displayLabel = loaderVersion;
		string type = "";
		string patch = "";
		if (loaderVersion.Contains('|'))
		{
			int pipeIdx = loaderVersion.IndexOf('|');
			displayLabel = loaderVersion.Substring(0, pipeIdx);
			filename = loaderVersion.Substring(pipeIdx + 1);
		}
		else
		{
			filename = $"OptiFine_{gameVersion}_{loaderVersion}.jar";
			displayLabel = loaderVersion;
		}
		if (displayLabel.Contains('_'))
		{
			int idx = displayLabel.LastIndexOf('_');
			type = displayLabel.Substring(0, idx);
			patch = displayLabel.Substring(idx + 1);
		}
		else
		{
			type = displayLabel;
		...
		string bmclUrl1 = $"https://bmclapi2.bangbang93.com/optifine/{filename}";
		string bmclUrl2 = $"https://bmclapi2.bangbang93.com/optifine/{gameVersion}/{type}/{patch}";
		string tempDir = Path.Combine(Path.GetTempPath(), "DIL");
		Directory.CreateDirectory(tempDir);
		string installerPath = Path.Combine(tempDir, filename);
		bool needDownload = !File.Exists(installerPath) || new FileInfo(installerPath).Length < 1024;
		if (needDownload)
		{
			if (task != null)
			{
				task.StepText = string.Format(LanguageManager.Get("DlModLoader"), "Optifine");
				task.TotalBytes = 0L;
				task.DownloadedBytes = 0L;
				DownloadManager.ProgressChanged?.Invoke(task);
			}
			string dlLabel = string.Format(LanguageManager.Get("DlModLoader"), "Optifine");
			bool downloaded = false;
			string lastError = "";
			foreach (string url in new[] { bmclUrl1, bmclUrl2 })
			{
				try
				{
					if (File.Exists(installerPath))
					...
					lastError = $"File too small ({(File.Exists(installerPath) ? new FileInfo(installerPath).Length : 0)} bytes) from {url}";
					if (File.Exists(installerPath))
						File.Delete(installerPath);
				}
				catch (Exception ex)
			...
			if (!downloaded)
			{
				throw new Exception(string.Format(LanguageManager.Get("ErrLoaderDownloadFail"), "Optifine", 2, lastError));
			}
		}
		string javaPath = FindJavaPath();
		if (string.IsNullOrEmpty(javaPath))
		...
		if (task != null)
		{
			task.StepText = string.Format(LanguageManager.Get("DlForgeInstalling"), "Optifine");
			DownloadManager.ProgressChanged?.Invoke(task);
		}
		string launchWrapperVersion = "1.12";
		using (var fs = File.OpenRead(installerPath))
		using (var archive = new ZipArchive(fs, ZipArchiveMode.Read))
		{
			var lwEntry = archive.Entries.FirstOrDefault(e => e.FullName.Equals("launchwrapper-of.txt", StringComparison.OrdinalIgnoreCase));
			if (lwEntry != null)
			{
				using var stream = lwEntry.Open();
				using var sr = new StreamReader(stream, Encoding.UTF8);
				launchWrapperVersion = (await sr.ReadToEndAsync()).Trim();
			}
			var lwJarEntry = archive.Entries.FirstOrDefault(x => x.FullName.Equals($"launchwrapper-of-{launchWrapperVersion}.jar"));
			if (lwJarEntry != null)
			{
				string lwDir = Path.Combine(MinecraftPath, "libraries", "optifine", "launchwrapper-of", launchWrapperVersion);
				Directory.CreateDirectory(lwDir);
				string lwPath = Path.Combine(lwDir, $"launchwrapper-of-{launchWrapperVersion}.jar");
				if (!File.Exists(lwPath))
				{
					using var lwStream = lwJarEntry.Open();
					using var lwFs = File.OpenWrite(lwPath);
					await lwStream.CopyToAsync(lwFs);
				}
			}
		}
		string lwLibName = launchWrapperVersion == "1.12"
			? "net.minecraft:launchwrapper:1.12"
			: $"optifine:launchwrapper-of:{launchWrapperVersion}";
		string optifineLibDir = Path.Combine(MinecraftPath, "libraries", "optifine", "Optifine", $"{gameVersion}_{editionRelease}");
		Directory.CreateDirectory(optifineLibDir);
		string optifineLibPath = Path.Combine(optifineLibDir, $"Optifine-{gameVersion}_{editionRelease}.jar");
		if (mergeIntoParent)
		{
			string parentJsonPath = Path.Combine(MinecraftPath, "versions", inheritsFrom, $"{inheritsFrom}.json");
			if (File.Exists(parentJsonPath))
			{
				string parentJson = await File.ReadAllTextAsync(parentJsonPath);
				using JsonDocument parentDoc = JsonDocument.Parse(parentJson);
				bool hasOptifineTweak = false;
				bool hasOptifineLib = false;
				if (parentDoc.RootElement.TryGetProperty("arguments", out var argsEl) && argsEl.TryGetProperty("game", out var gameArgsEl))
				{
					foreach (var arg in gameArgsEl.EnumerateArray())
					{
						if (arg.ValueKind == JsonValueKind.String && arg.GetString()?.Contains("optifine", StringComparison.OrdinalIgnoreCase) == true)
						{
							hasOptifineTweak = true;
							break;
						}
					}
				}
				if (parentDoc.RootElement.TryGetProperty("libraries", out var libsEl))
				{
					foreach (var lib in libsEl.EnumerateArray())
					{
						if (lib.TryGetProperty("name", out var nameEl))
						{
							string libName = nameEl.GetString() ?? "";
							if (libName.Contains("optifine", StringComparison.OrdinalIgnoreCase))
							{
								hasOptifineLib = true;
								break;
							}
				...
				if (!hasOptifineTweak || !hasOptifineLib)
				{
					...
					if (node != null)
					{
						if (!hasOptifineTweak)
						{
							...
							if (args != null)
							{
								var gameArgs = args["game"];
								if (gameArgs != null)
								{
									var gameArr = gameArgs.AsArray();
						...
						if (!hasOptifineLib)
						{
							var libs = node["libraries"];
							if (libs != null)
							{
								var libsArr = libs.AsArray();
								var lib1 = new JsonObject { ["name"] = lwLibName };
								var lib2 = new JsonObject { ["name"] = $"optifine:Optifine:{gameVersion}_{editionRelease}" };
								libsArr.Add(lib1);
								libsArr.Add(lib2);
							}
						}
						string updatedJson = node.ToJsonString(new JsonSerializerOptions { WriteIndented = true, TypeInfoResolver = System.Text.Json.Serialization.Metadata.JsonTypeInfoResolver.Combine(System.Text.Json.JsonSerializerOptions.Default.TypeInfoResolver) });
						await File.WriteAllTextAsync(parentJsonPath, updatedJson);
					}
				}
			}
		}
		...
		{
			string optifineId = $"{gameVersion}-Optifine_{editionRelease}";
			string versionDir = Path.Combine(MinecraftPath, "versions", optifineId);
			Directory.CreateDirectory(versionDir);
			var versionJson = new
			{
				id = optifineId,
				...
				arguments = new
				{
					game = new[] { "--tweakClass", "optifine.OptiFineTweaker" },
					jvm = new string[0]
				},
				...
				{
					new { name = lwLibName },
					new { name = $"optifine:Optifine:{gameVersion}_{editionRelease}" }
				},
				mainClass = "net.minecraft.launchwrapper.Launch",
				minimumLauncherVersion = 21
			};
			string versionJsonPath = Path.Combine(versionDir, $"{optifineId}.json");
			string jsonStr = JsonSerializer.Serialize(versionJson, new JsonSerializerOptions { WriteIndented = true, PropertyNamingPolicy = JsonNamingPolicy.CamelCase });
			await File.WriteAllTextAsync(versionJsonPath, jsonStr);
		}
		string gameJarPath = ResolveGameJarPath(inheritsFrom);
		string patcherWorkDir = Path.Combine(Path.GetTempPath(), "DIL", "patcher");
		Directory.CreateDirectory(patcherWorkDir);
		string patcherGameJar = Path.Combine(patcherWorkDir, "minecraft.jar");
		string patcherOptifineJar = Path.Combine(patcherWorkDir, "optifine.jar");
		string patcherOutputJar = Path.Combine(patcherWorkDir, "optifine-patched.jar");
		File.Copy(gameJarPath, patcherGameJar, true);
		File.Copy(installerPath, patcherOptifineJar, true);
		ProcessStartInfo psi = new ProcessStartInfo
		{
			FileName = javaPath,
			Arguments = $"-Dfile.encoding=UTF-8 -Dsun.jnu.encoding=UTF-8 -cp \"{patcherOptifineJar}\" optifine.Patcher \"{patcherGameJar}\" \"{patcherOptifineJar}\" \"{patcherOutputJar}\"",
			UseShellExecute = false,
			RedirectStandardOutput = true,
			RedirectStandardError = true,
			CreateNoWindow = true,
			WorkingDirectory = patcherWorkDir
		};
		Process proc = Process.Start(psi);
		if (proc == null)
		{
			throw new Exception("Failed to start OptiFine Patcher process");
		}
		StringBuilder stdoutBuilder = new StringBuilder();
		List<string> errList = new List<string>();
		proc.OutputDataReceived += (s, e) => { if (!string.IsNullOrEmpty(e.Data)) stdoutBuilder.AppendLine(e.Data); };
		proc.ErrorDataReceived += (s, e) => { if (!string.IsNullOrEmpty(e.Data)) errList.Add(e.Data); };
		proc.BeginOutputReadLine();
		proc.BeginErrorReadLine();
		await proc.WaitForExitAsync();
		if (errList.Count > 0)
		{
			throw new Exception($"OptiFine Patcher failed: {string.Join("\n", errList)}");
		}
		if (File.Exists(patcherOutputJar))
		{
			File.Copy(patcherOutputJar, optifineLibPath, true);
		}
		try { File.Delete(patcherGameJar); File.Delete(patcherOptifineJar); File.Delete(patcherOutputJar); } catch { }
		if (task != null)
		{
			task.StepText = string.Format(LanguageManager.Get("DlLoaderInstalled"), "Optifine");
			DownloadManager.ProgressChanged?.Invoke(task);
		}
	}
}