...
public static class DownloadManager
{
	...
	public static void StartDownload(string versionId, string loaderName = "", string loaderVersion = "", string optifineVersion = "")
	{
		if (!IsDownloading)
		{
			...
			DownloadManager.ProgressChanged?.Invoke(CurrentTask);
			StartSpeedMonitor();
			DownloadAsync(versionId, loaderName, loaderVersion, optifineVersion);
		}
	}
	...
	private static async Task DownloadAsync(string versionId, string loaderName, string loaderVersion, string optifineVersion)
	{
		try
		{
			...
			if (!string.IsNullOrEmpty(optifineVersion))
		{
			UpdateStep(DownloadStep.DownloadingModLoader, string.Format("Optifine {0}", optifineVersion), 92.0, 0, 0);
			...
			string inheritsFrom = hasMainLoader
				...
				: versionId;
			await InstallOptifineLoader(client, versionId, optifineVersion, inheritsFrom, hasMainLoader);
		}
	...
	private static async Task DownloadModLoader(HttpClient client, string versionId, string loaderName, string loaderVersion)
	{
		...
		case "Quilt":
			...
			break;
		case "Optifine":
			await InstallOptifineLoader(client, versionId, loaderVersion, versionId);
			break;
		default:
	...
	private static async Task InstallOptifineLoader(HttpClient client, string gameVersion, string loaderVersion, string inheritsFrom, bool mergeIntoParent = false)
	{
		string filename = "";
		...
		if (needDownload)
		{
			if (CurrentTask != null)
			{
				CurrentTask.StepText = string.Format(LanguageManager.Get("DlModLoader"), "Optifine");
				CurrentTask.TotalBytes = 0L;
				CurrentTask.DownloadedBytes = 0L;
				DownloadManager.ProgressChanged?.Invoke(CurrentTask);
			}
		...
		if (CurrentTask != null)
		{
			CurrentTask.StepText = string.Format(LanguageManager.Get("DlForgeInstalling"), "Optifine");
			DownloadManager.ProgressChanged?.Invoke(CurrentTask);
		}
		...
		if (CurrentTask != null)
		{
			CurrentTask.StepText = string.Format(LanguageManager.Get("DlLoaderInstalled"), "Optifine");
			DownloadManager.ProgressChanged?.Invoke(CurrentTask);
		}
	}
}