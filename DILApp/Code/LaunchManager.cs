...
public static class LaunchManager
{
	...
	public static event Action<Exception, string>? LaunchFailed;
	...
	private static void EnsureVersionJar(string versionId)
	{
		string versionsDir = Path.Combine(MinecraftPath, "versions");
		string versionJar = Path.Combine(versionsDir, versionId, versionId + ".jar");
		if (File.Exists(versionJar))
		{
			return;
		}
		string versionJson = Path.Combine(versionsDir, versionId, versionId + ".json");
		...
		try
		{
			string jsonContent = File.ReadAllText(versionJson);
			using JsonDocument doc = JsonDocument.Parse(jsonContent);
			string jarId = null;
			if (doc.RootElement.TryGetProperty("jar", out var jarProp) && jarProp.ValueKind == JsonValueKind.String)
			{
				jarId = jarProp.GetString();
			}
			else if (doc.RootElement.TryGetProperty("inheritsFrom", out var inheritsProp) && inheritsProp.ValueKind == JsonValueKind.String)
			{
				jarId = inheritsProp.GetString();
			}
			if (string.IsNullOrEmpty(jarId) || jarId == versionId)
			{
				return;
			}
			string sourceJar = Path.Combine(versionsDir, jarId, jarId + ".jar");
			if (!File.Exists(sourceJar))
			{
				return;
			}
			Directory.CreateDirectory(Path.GetDirectoryName(versionJar));
			File.Copy(sourceJar, versionJar, true);
		}
		catch
		{
		}
		...
		foreach (string text3 in array2)
		{
			...
			try
			{
				...
				if (rootElement.TryGetProperty("mainClass", out var value3))
				{
					...
					if (text4.Contains("launchwrapper", StringComparison.OrdinalIgnoreCase))
					{
						try
						{
							if (rootElement.TryGetProperty("arguments", out var argsEl) && argsEl.TryGetProperty("game", out var gameArgsEl))
							{
								foreach (var argEl in gameArgsEl.EnumerateArray())
								{
									if (argEl.ValueKind == JsonValueKind.String && argEl.GetString()?.Contains("optifine", StringComparison.OrdinalIgnoreCase) == true)
									{
										installedVersion.IsOptifine = true;
										break;
									}
								}
							}
				...
				if (!installedVersion.IsForge && !installedVersion.IsFabric && !installedVersion.IsQuilt)
				{
					if (rootElement.TryGetProperty("libraries", out var libsEl2))
					{
						foreach (var lib in libsEl2.EnumerateArray())
						{
							if (lib.TryGetProperty("name", out var libNameEl))
							{
								string libName = libNameEl.GetString() ?? "";
								if (libName.Contains("minecraftforge", StringComparison.OrdinalIgnoreCase) || libName.Contains("net.minecraftforge", StringComparison.OrdinalIgnoreCase))
								{
									installedVersion.IsForge = true;
									break;
								}
								if (libName.Contains("net.fabricmc", StringComparison.OrdinalIgnoreCase) || libName.Contains("fabric-loader", StringComparison.OrdinalIgnoreCase))
								{
									installedVersion.IsFabric = true;
									break;
								}
								if (libName.Contains("org.quiltmc", StringComparison.OrdinalIgnoreCase) || libName.Contains("quilt-loader", StringComparison.OrdinalIgnoreCase))
								{
									installedVersion.IsQuilt = true;
									break;
								}
				...
				if (!installedVersion.IsOptifine)
				{
					try
					{
						if (rootElement.TryGetProperty("arguments", out var argsEl2) && argsEl2.TryGetProperty("game", out var gameArgsEl2))
						{
							foreach (var argEl in gameArgsEl2.EnumerateArray())
							{
								if (argEl.ValueKind == JsonValueKind.String && argEl.GetString()?.Contains("optifine", StringComparison.OrdinalIgnoreCase) == true)
								{
									installedVersion.IsOptifine = true;
				...
				if (rootElement.TryGetProperty("inheritsFrom", out var value4))
				{
					...
					if (!string.IsNullOrEmpty(string2))
					{
						if (!installedVersion.IsForge && !installedVersion.IsFabric && !installedVersion.IsQuilt)
						{
							string parentJsonPath = Path.Combine(text, string2, string2 + ".json");
							if (File.Exists(parentJsonPath))
							{
								try
								{
									using JsonDocument parentDoc = JsonDocument.Parse(File.ReadAllText(parentJsonPath));
									JsonElement parentRoot = parentDoc.RootElement;
									if (parentRoot.TryGetProperty("mainClass", out var parentMainClass))
									{
										string parentMC = parentMainClass.GetString() ?? "";
										if (parentMC.Contains("forge", StringComparison.OrdinalIgnoreCase))
										{
											installedVersion.IsForge = true;
										}
										if (parentMC.Contains("fabric", StringComparison.OrdinalIgnoreCase))
										{
											installedVersion.IsFabric = true;
										}
										if (parentMC.Contains("quilt", StringComparison.OrdinalIgnoreCase))
										{
											installedVersion.IsQuilt = true;
										}
									}
									if (!installedVersion.IsForge && !installedVersion.IsFabric && !installedVersion.IsQuilt)
									{
										if (parentRoot.TryGetProperty("libraries", out var parentLibs))
										{
											foreach (var lib in parentLibs.EnumerateArray())
											{
												if (lib.TryGetProperty("name", out var libNameEl))
												{
													string libName = libNameEl.GetString() ?? "";
	...
	public static async Task LaunchAsync(string versionId, string username, int maxRamMb = 2048, bool forceCheckFiles = false, MSession? msSession = null)
	{
		if (IsLaunching)
		{
		...
		try
		{
			...
			MSession session = msSession ?? await CreateSessionForSkinAsync(username, skinType, skinId);
			...
			string? foundJava = DownloadManager.FindJavaPathForVersion(versionId);
			if (!string.IsNullOrEmpty(foundJava))
			{
				launchOption.JavaPath = foundJava;
			}
		...
		catch (Exception ex3)
		{
			...
			try
			{
				...
				sb.AppendLine();
				string logDir = Path.Combine(DownloadManager.GetExeDirectory(), "logs");
				Directory.CreateDirectory(logDir);
				logFilePath = Path.Combine(logDir, $"launch_fail_{DateTime.Now:yyyyMMdd_HHmmss}.txt");
				File.WriteAllText(logFilePath, sb.ToString());
			}
			catch
			{
			}
			LaunchManager.LaunchFailed?.Invoke(ex, logFilePath);
		}
		finally
		{
			IsLaunching = false;
	...
	private static void StartProcessWithCapture(Process process)
	{
		try
		{
			...
			{
				string shortName = DownloadManager.GetShortPath(fileName);
				if (!string.IsNullOrEmpty(shortName))
				{
					process.StartInfo.FileName = shortName;
				}
			}
		}
		catch
		...
		try
		{
			...
			string fixedArgs = FixClasspath(args, process);
			if (!string.IsNullOrEmpty(fixedArgs))
			{
				process.StartInfo.Arguments = fixedArgs;
			}
		}
		catch
		{
	...
	private static string FixClasspath(string args, Process process)
	{
		if (args.Length <= 8000)
		{
			return args;
		...
		{
			int idx2 = args.IndexOf("-classpath ");
			if (idx2 >= 0)
			{
				cpFlagIdx = idx2;
				cpFlagLen = 11;
		...
		int valueStart = cpFlagIdx + cpFlagLen;
		...
		if (args[valueStart] == '"')
		{
			int closeQuote = args.IndexOf('"', valueStart + 1);
			if (closeQuote < 0)
			{
				return args;
			}
			classpathValue = args.Substring(valueStart + 1, closeQuote - valueStart - 1);
			valueEnd = closeQuote + 1;
		}
		else
		{
			int spaceIdx = args.IndexOf(' ', valueStart);
			if (spaceIdx < 0)
			{
				classpathValue = args.Substring(valueStart);
				valueEnd = args.Length;
			}
			else
			{
				classpathValue = args.Substring(valueStart, spaceIdx - valueStart);
				valueEnd = spaceIdx;
			}
		}
		if (string.IsNullOrEmpty(classpathValue))
		{
			return args;
		}
		process.StartInfo.Environment["CLASSPATH"] = classpathValue;
		string beforeCp = args.Substring(0, cpFlagIdx);
		string afterCp = args.Substring(valueEnd);
